  ...
  <div class="sidebar">
      ...
      <div class="sidebar-menu">
          ...
          <ul class="dropDown">
              ...
              <ul class="dropDown">
                  <li class="active" id="nested111" class="activeNested activeNestedStyle"><a href="takmicari.php">Pojedinačno</a></li>
  ...
  <div class="main-content">
      ...
      ?>
        <main>
          ...
          <?php
              ...
              if(isset($_POST['search']) && !$_POST['search']=="")
              {
                ...
                $sql="SELECT * FROM competitor_table
                WHERE ime LIKE '%$searchKey%'
                OR prezime LIKE '%$searchKey%'
                OR uloga LIKE '%$searchKey%'
                OR disciplina LIKE '%$searchKey%'
                ";
              }
              else
              {
                $sql="SELECT * FROM competitor_table LIMIT $start_from, $num_per_page";
                $searchKey = "";
              }
        ...
        <div class="search-add">
            ...
            <button >
              <a href="addCompetitor.php" style="color: white;">
                Dodaj takmičara
              </a>
            </button>
        </div>
        ...
        <table class="table">
                <tbody>
                    <tr>
                        ...
                        <form action="updateCompetitor.php" method="post" >
                            <input type="hidden" type="submit" name="id" placeholder="Dodaj" value="<?= $rows['id'];?>">
                            <button
                                type="submit" name="update_competitor"
                        ...
                        <form action="code.php" method="post">
                            <input type="hidden" type="submit" name="delete_competitor_id" placeholder="Izbriši" value="<?= $rows['id'];?>">
                            <input type="hidden" type="submit" name="user_email" placeholder="Email" value="<?= $rows['email'];?>">
                            <button
                                type="submit" name="delete_competitor" onclick="return deleteCompetitor()"
            ...
            <?php
                // PAGINATION
                $pr_query = "SELECT * FROM competitor_table";
            ...
            <?php
                if($page>=2)
                {
                    echo "<a href='competitor.php?page=".($page-1)."' class='btn-pagination'>&laquo;</a>";
                }
                ...
                for($i = 1; $i <= $total_page; $i++)
                {
                    if ($i == $page)
                    {
                        echo "<a class = 'btn-pagination active' href='competitor.php?page=".$i."'>".$i." </a>";
                    }

                    else
                    {
                        echo "<a href='competitor.php?page=".$i."' class='btn-pagination'>$i</a>";
                    }
                }

                if($page<$total_page)
                {
                    echo "<a href='competitor.php?page=".($page+1)."' class='btn-pagination'>&raquo;</a>";
                }
            ?>
            </div>
        </main>
